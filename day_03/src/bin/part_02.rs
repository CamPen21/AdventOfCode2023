...
pub fn gear_ratios(input: Vec<Vec<char>>) -> Result<u32, ()> {
    ...
    let mut pairs: HashMap<(usize, usize), Vec<u32>> = HashMap::new();
    for y in 0..input.len() {
        let mut buffer = String::new();
        let mut check = true;
        let mut ast: Option<(usize, usize)> = None;
        for x in 0..input[0].len() {
            ...
            if !is_number && !check{
                ...
                if let Ok(number) = res {
                    numbers.push(number);
                    let ent = pairs.entry(ast.unwrap()).or_insert(vec![]);
                    ent.push(number);
                    ast = None;
                }
            }
            ...
            if is_number && check {
                let is_part_number: bool = DIRECTIONS.iter().fold(false, |acc, dir| {
                    ...
                    match _char {
                        Some(found) => {
                            if is_asterisk(found) {
                                ast = Some(((y as isize + dir.r) as usize, (x as isize + dir.c) as usize));
                            }
                            acc || is_asterisk(found)
                            },
                        None => acc
                    }
                });

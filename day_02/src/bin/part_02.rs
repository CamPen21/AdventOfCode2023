...
pub fn games_sum(input: Vec<&str>) -> Result<u32, ()> {
    ...
    for game in games {
        ...
        cubes_left.insert("red", 0);
        cubes_left.insert("green", 0);
        cubes_left.insert("blue", 0);
        for set in &game.sets {
            let red_cubes = cubes_left.entry("red").or_insert(set.red);
            *red_cubes = cmp::max(*red_cubes, set.red);
            let blue_cubes = cubes_left.entry("blue").or_insert(set.blue);
            *blue_cubes = cmp::max(*blue_cubes, set.blue);
            let green_cubes = cubes_left.entry("green").or_insert(set.green);
            *green_cubes = cmp::max(*green_cubes, set.green);
        }
        counter += cubes_left.get("red").unwrap() * cubes_left.get("blue").unwrap() * cubes_left.get("green").unwrap();
    }
    Ok(counter)
}
